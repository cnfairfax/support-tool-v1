...
class CheckForm extends React.Component {
    ...
    handleSubmit(event) {
        ...
        axios.post('https://cd-tool-cnfairfax.c9users.io:8082/api/checkForBlockedEmails', {
            accountKey: this.state.accountKey,
            token: this.state.token,
            subDomain: this.state.subd,
            checkEmails: this.state.emailsToCheck
        }).then((response) => {
            console.log(response.data);
            this.setState({ processing: false, result: 'success', reportData: response.data });
        }).catch((error) => {
            if (error.response) {
    ...
    render() {
        ...
        const { result, processing, reportData, accountKey, subd, token } = this.state
        
        if(result === 'success') {
            return (
                <Redirect to={{
                    pathname: '/BlockedReport',
                    state: { 
                        data: reportData,
                        auth: {
        ...
        return (
            <form onSubmit={this.handleSubmit}>
                ...
                <div className="field-group">
                    <label htmlFor="emailsToCheck">Comma Separated Email Addresses:</label>
                    <textarea name="emailsToCheck" id="emailsToCheck" onChange={this.handleChange}/>
                </div>
